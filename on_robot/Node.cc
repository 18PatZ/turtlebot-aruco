...
void Node::LoadOrbParameters (ORB_SLAM2::ORBParameters& parameters) {
  ...
  bool invert_camera_calibration_baseline = false;
  node_handle_.param(name_of_node_ + "/invert_camera_calibration_baseline", invert_camera_calibration_baseline, false);
  ...
  if (load_calibration_from_cam) {
    ...
    if(camera_info == nullptr){
      ...
      if(invert_camera_calibration_baseline){
        parameters.baseline = -1 * parameters.baseline;
        ROS_INFO_STREAM ("Inverted baseline value from camera calibration, now set to " << parameters.baseline);
      }
